...
const WishListProducts = () => {
  ...
  const loadMore = () => {
    setVisibleProducts((prev) =>
      Math.min(prev + 5, favoriteProduct?.length ?? 0)
    );
  };

  const handleResetWishlist = () => {
    const confirmReset = window.confirm(
      "Are you sure that you want to reset your wishlist?"
    );
    if (confirmReset) {
      resetFavorite();
  ...
  return (
    <Container>
      {favoriteProduct?.length > 0 ? (
        <>
          <div className="overflow-x-auto">
            ...
            <div className="flex gap-4 mt-4">
              {visibleProducts < (favoriteProduct?.length ?? 0) && (
        ...
        <div className="flex min-h-400 flex-col items-center justify-center scroll-smooth space-y-6 px-4 text-center">
