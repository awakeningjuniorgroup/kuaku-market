...
import { Calendar, ChevronLeftIcon, Heart, Pencil } from "lucide-react";
        ...
        <div className="md:col-span-3">
          {blog?.mainImage && (
            <Image
              ...
              width={800}
              height={800}
              className="w-full max-h-500 object-cover rounded-lg"
            />
          )}
          <div>
            <div className="text-xs flex items-center gap-5 my-7">
              <div className="flex items-center relative group cursor-pointer">
                {blog?.blogcategories?.map(
                  (item, index: number) => (
                    <p
                      key={index}
                      className="font-semibold text-xl text-shop_dark_green tracking-wider"
                    >
                      {item?.title}
                    </p>
              ...
              <p className="flex items-center gap-1 text-lightColor relative group hover:cursor-pointer hover:text-shop_dark_green hoverEffect">
                <Pencil size={35} /> {blog?.author?.name}
                  ...
                  {blog.body && (
                    <PortableText
                      ...
                      components={{
                        block: {
                          normal: ({ children }) => (
                            <h5 className="my-5 text-base/8 first:mt-0 last:mb-0">
                              {children}
                            </h5>
                          ),
                          h2: ({ children }) => (
...
const BlogLeft = async ({ slug }: { slug: string }) => {
  ...
  return (
    <div  style={{
      ...
      >
      <div className="border border-lightColor p-5 rounded-md scroll-smooth bg-white ">
